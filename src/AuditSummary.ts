...
export default class AuditSumary {
    ...
    public executeAudit() {
        ...
        if (vulnerabilityList) {
            ...
            const globalCounts: PerRootCounts = { ...global, total: globalTotal };

            // Output
            if (jsonMode) {
                this.outputJSON(perRootCounts, globalCounts);
            } else {
                this.outputTerminal(byRoot, perRootCounts, globalCounts);
            }
        }
    }

    private outputTerminal(byRoot: Map<string, VulnerabilityView[]>, perRootCounts: Map<string, { vulns: VulnerabilityView[]; counts: PerRootCounts;}>, globalCounts: PerRootCounts) {
        ...
        function coloredSeverity(severity: Severity) {
            switch (SEV_ORDER[severity]) {
                case 4: return chalk.magentaBright.bold(severity);
                case 3: return chalk.redBright.bold(severity);
                case 2: return chalk.yellowBright.bold(severity);
        ...
        rowArrays.push(...this.getAllRootsRowData(rootsByTotalDesc));
        rowArrays.push(['','','','','','','']);
        ...
        const table = new AsciiTable3('NPM Packages Audit Summary (deduped by package per root dependency)')
            .setHeading('Root Dependency', 'Critical', 'High', 'Moderate', 'Low', 'Info', 'Total')
    ...
    private getAllRootsRowData(rootsByTotalDesc: [string, { vulns: VulnerabilityView[]; counts: PerRootCounts; }][]) {
        const rowArrays = [];

        for (const [root, { counts }] of rootsByTotalDesc) {
            const coloredRoot = counts.critical ? chalk.magentaBright.bold(root) : counts.high ? chalk.redBright.bold(root) : counts.moderate ? chalk.yellowBright.bold(root) : root;
            const data = [
                coloredRoot, 
     ...
     * @returns 
     */
    private outputJSON(perRootCounts: Map<string, { vulns: VulnerabilityView[]; counts: PerRootCounts;}>, globalCounts: PerRootCounts) {
        const rootsJson = Object.fromEntries(
            Array.from(perRootCounts.entries()).map(([root, { vulns, counts }]) => [
                root,
                { vulnerabilities: vulns, summary: counts },
            ])
        );
    ...
    private getVulnerabilitiesPerRootCount(vulnerabilitiesByRoot: Map<string, VulnerabilityView[]>) {
        ...
        for (const [root, list] of vulnerabilitiesByRoot.entries()) {
            ...
            perRootCounts.set(root, { vulns: dedup, counts: { ...counts, total } });
        }
    ...
    private getVulnerabilityList(audit: AuditJson, jsonMode: boolean) {
        ...
        if (vulnList.length === 0) {
            if (jsonMode) {
                ...
                const emptyCounts: PerRootCounts = { critical: 0, high: 0, moderate: 0, low: 0, info: 0, total: 0 };
